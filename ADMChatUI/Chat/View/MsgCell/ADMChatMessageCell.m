...
#import "ADMChatMessageCell.h"
#import "ADMChatBtn.h"
#import "WZChatMacro.h"
#import "UIView+ADMChat.h"
#import "ADMChatHelper.h"

@implementation ADMChatMessageCell {
    ADMChatBtn *_retryBtn;
}
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    ...
    if (self) {
        ...
        _retryBtn = [ADMChatBtn chatButtonWithType:ADMChatButtonTypeRetry];
        [_retryBtn setImage:[ADMInputHelper otherImageNamed:@"ADM_chat_retry"] forState:UIControlStateNormal];
...
- (void)setConfig:(ADMChatMessageModel *)model isShowName:(BOOL)isShowName {
    ...
    if (model.isSender) {
        ...
        _bubbleImageView.image = [ADMChatHelper senderBubble];
        
        //消息内容
        CGRect rect = _bubbleImageView.frame;
        if (model.msgType == ADMMessageTypeText) {
            rect.origin.x += 5;
            rect.size.width -= 17;
        }
        _contentRect = rect;
        ...
        if (model.sendType == ADMMessageSendTypeWaiting) {
            _activityView.hidden = NO;
            [_activityView startAnimating];
            
            _retryBtn.hidden = YES;
        }
        else if (model.sendType == ADMMessageSendTypeSuccess) {
        ...
        _bubbleImageView.image = [ADMChatHelper receiverBubble];
