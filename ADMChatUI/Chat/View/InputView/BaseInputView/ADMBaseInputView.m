...
@interface ADMBaseInputView ()<UITextViewDelegate,UITextFieldDelegate>
...
///当前键盘类型
@property (nonatomic, assign) ADMKeyboardType type;
///当前键盘索引, -1为z系统键盘
...
@implementation ADMBaseInputView
...
- (void)keyboardValueChange:(NSNotification *)notification {
    ...
    if (beginFrame.origin.y < endFrame.origin.y) {
        if (self.keyboardIndex == -1) {
            ...
            //自定义键盘弹出
            [self ADM_showKeyboardAtIndex:self.keyboardIndex duration:duration];
        }
    }
    else {
        //系统键盘弹出
        ...
        self.keyboardIndex = -1;
        self.type = ADMKeyboardTypeSystem;
...
- (void)minYWillChange:(CGFloat)minY duration:(CGFloat)duration dismissKeyboard:(BOOL)dismissKeyboard {
    ...
    if (minY < self.startY) {
        minY += [ADMInputHelper helper].iPhoneXBottomH;
        self.keyboardH -= [ADMInputHelper helper].iPhoneXBottomH;
    }
...
- (void)showSystemKeyboard {
    if (self.type != ADMKeyboardTypeSystem) {
        self.type = ADMKeyboardTypeSystem;
        if (self.inputView1) {
...
- (void)showKeyboardAtIndex:(NSInteger)index duration:(CGFloat)duration {
    ...
    if (self.type == ADMKeyboardTypeSystem) {
        ...
        self.keyboardIndex = index;
        //直接弹出自定义键盘
        [self ADM_showKeyboardAtIndex:index duration:duration];
    }
}

- (void)dismissKeyboard {
    if (self.type == ADMKeyboardTypeIdle) return;
    if (self.type == ADMKeyboardTypeSystem) {
        [self endEditing:YES];
    }
    else {
...
//直接弹出自定义键盘
- (void)ADM_showKeyboardAtIndex:(NSInteger)index duration:(CGFloat)duration {
    //直接弹出自定义键盘
    self.type = ADMKeyboardTypeOther;
    UIView *k;
    ...
    if (self.inputView1) {
        if (self.inputView1.text.length > 0) {
            ...
            if (length > 0) {
                ...
                NSString *emoticon = [[ADMEmoticonManager manager] willDeleteEmoticon:subString];
                if ([[ADMEmoticonManager manager].chs containsObject:emoticon]) {
