//
//  ADMEmoticonManager.m
//  ADMChat
//
//  Created by WangKai on 2019/5/17.
//  Copyright © 2021 WangKai. All rights reserved.
//

#import "ADMEmoticonManager.h"
#import "ADMInputHelper.h"
#import "ADMInputBase64.h"

@interface ADMEmoticonManager ()
...
@implementation ADMEmoticonManager

+ (instancetype)manager {
    static ADMEmoticonManager *m;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        m = [[ADMEmoticonManager alloc] init];
    });
    return m;
}
...
- (void)loadEmoticons {
    ...
    //默认表情
    NSString *path1 = [[NSBundle mainBundle] pathForResource:@"ADMEmoticon1" ofType:@"plist"];
    NSArray *array1 = [[NSArray alloc] initWithContentsOfFile:path1];
    [emoticons addObject:array1];
    
    //浪小花
    NSString *path2 = [[NSBundle mainBundle] pathForResource:@"ADMEmoticon2" ofType:@"plist"];
    NSArray *array2 = [[NSArray alloc] initWithContentsOfFile:path2];
    [emoticons addObject:array2];
    //emojis
    NSString *emojisPath = [[NSBundle mainBundle] pathForResource:@"ADMEmojis" ofType:@"plist"];
...
- (NSMutableAttributedString *)attributedString:(NSString *)aString {
    ...
    if (!error) {
        ...
        for (NSTextCheckingResult *match in matchs) {
            ...
            UIImage *image = [ADMInputHelper emoticonImageNamed:imageName];
            if (image) {
