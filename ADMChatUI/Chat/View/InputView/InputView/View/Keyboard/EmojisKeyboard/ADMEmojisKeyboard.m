...
#import "ADMEmojisKeyboard.h"
#import "ADMBlankCell.h"
#import "ADMEmojisCell.h"
#import "ADMDeleteCell.h"
#import "ADMEmoticonCell.h"
#import "ADMHorizontalLayout.h"
#import "ADMEmoticonManager.h"
#import "ADMInputHelper.h"

#define key_rows  3
#define key_nums  7
@interface ADMEmojisKeyboard ()<UICollectionViewDelegate,UICollectionViewDataSource>

@end

@implementation ADMEmojisKeyboard {
    NSArray *_btns;
    NSArray *_emoticons;
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        //加载表情
        _emoticons = [ADMEmoticonManager manager].emoticons;
        _emojisSection = _emoticons.count-1;
        
        CGFloat key_itemW = 45;
        CGFloat spcing = (320-key_itemW*key_nums)/(key_nums+1);
        ADMHorizontalLayout *horLayout = [[ADMHorizontalLayout alloc] initWithSpacing:spcing rows:key_rows nums:key_nums];
        
        CGRect rect = self.bounds;
        rect.size.height -= (40+[ADMInputHelper helper].iPhoneXBottomH);
        ...
        [_collectionView registerClass:[ADMBlankCell class] forCellWithReuseIdentifier:@"blank"];
        [_collectionView registerClass:[ADMEmojisCell class] forCellWithReuseIdentifier:@"emojis"];
        [_collectionView registerClass:[ADMDeleteCell class] forCellWithReuseIdentifier:@"delete"];
        [_collectionView registerClass:[ADMEmoticonCell class] forCellWithReuseIdentifier:@"emoticon"];
        [self addSubview:_collectionView];
        
        UIColor *themeColor = [UIColor colorWithRed:34/255. green:207/255. blue:172/255. alpha:1];
        UIView *toolView = [[UIView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(_collectionView.frame), frame.size.width, 40+[ADMInputHelper helper].iPhoneXBottomH)];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    if ([self isDelete:indexPath.item]) {
        ADMDeleteCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"delete" forIndexPath:indexPath];
        return cell;
    }
    else {
        ...
        if (index < emojis.count) {
            if (indexPath.section == _emojisSection) {
                ...
                ADMEmojisCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"emojis" forIndexPath:indexPath];
                [cell setConfig:text];
                return cell;
            }
            else {
                ...
                ADMEmoticonCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"emoticon" forIndexPath:indexPath];
                [cell setConfig:[dic objectForKey:@"png"]];
                return cell;
            }
        }
        else {
            ADMBlankCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"blank" forIndexPath:indexPath];
            return cell;
        }
    }
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    NSInteger index = scrollView.contentOffset.x/[ADMInputHelper helper].screenW;
...
- (void)toolBtnClick:(UIButton *)btn {
    ...
    [_collectionView setContentOffset:CGPointMake([ADMInputHelper helper].screenW*index, 0) animated:NO];
}
