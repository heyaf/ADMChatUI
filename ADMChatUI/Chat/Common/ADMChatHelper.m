...
@implementation ADMChatHelper

+ (instancetype)helper {
    static ADMChatHelper *helper;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        helper = [[ADMChatHelper alloc] init];
    });
    return helper;
}
...
+ (UIImage *)senderBubble {
    return [ADMChatHelper helper].senderBubbleImage;
}

+ (UIImage *)receiverBubble {
    return [ADMChatHelper helper].receiverBubbleImage;
}
...
+ (UIImage *)getImageFromCacheWithUrl:(NSString *)url {
    //1、从内存存获取
    ADMChatHelper *helper = [ADMChatHelper helper];
...
- (UIImage *)senderBubbleImage {
    if (_senderBubbleImage == nil) {
        UIImage *image = [ADMInputHelper otherImageNamed:@"ADM_chat_bj2"];
...
- (UIImage *)receiverBubbleImage {
    if (_receiverBubbleImage == nil) {
        UIImage *image = [ADMInputHelper otherImageNamed:@"ADM_chat_bj1"];
        CGSize size = image.size;
