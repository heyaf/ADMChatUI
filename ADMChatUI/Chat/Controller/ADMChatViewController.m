...
#import "ADMChatViewController.h"
#import "ADMChatSystemCell.h"
#import "ADMChatTextMessageCell.h"
#import "ADMChatVoiceMessageCell.h"
#import "ADMChatImageMessageCell.h"
#import "ADMChatVideoMessageCell.h"

@interface ADMChatViewController ()<UITableViewDelegate,UITableViewDataSource,UIGestureRecognizerDelegate,ADMInputViewDelegate>
...
///当前私聊用户
@property (nonatomic, strong) ADMChatUserModel *userModel;
///当前群聊
@property (nonatomic, strong) ADMChatGroupModel *groupModel;
///私聊消息是否显示昵称, 不需要手动设置, 会根据私聊或者群聊用户自动判断, 此处只做记录
@property (nonatomic, assign, getter=isShowName) BOOL showName;
///自定义表情键盘
@property (nonatomic, strong) ADMInputView *inputView;
///手势处理相关
...
@implementation ADMChatViewController
...
- (instancetype)initWithSession:(ADMChatSessionModel *)sessionModel {
    self = [super init];
    if (self) {
        [self setConfig:[[ADMChatDBManager DBManager] selectChatModel:sessionModel]];
    }
    return self;
}

- (void)setConfig:(ADMChatBaseModel *)model {
    self.title = @"消息";
    self.tableViewY = CHAT_NAV_BAR_H;
    if ([model isKindOfClass:[ADMChatUserModel class]]) {
        self.userModel = (ADMChatUserModel *)model;
        self.showName = self.userModel.isShowName;
    }
    else {
        self.groupModel = (ADMChatGroupModel *)model;
...
- (void)loadMessage:(NSInteger)page {
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        if (self.userModel) {
            self.messageModels = [[ADMChatDBManager DBManager] messagesWithUser:self.userModel];
        }
        else {
            self.messageModels = [[ADMChatDBManager DBManager] messagesWithGroup:self.groupModel];
        }
...
- (void)rightItemClick {
    ...
    if (msgType == ADMMessageTypeSystem) {
        NSString *timeMessage = [ADMChatHelper timeFromDate:[NSDate date]];
        ADMChatMessageModel *model = [ADMChatMessageManager createSystemMessage:self.userModel
                                                                        message:timeMessage
                                                                       isSender:YES];
        [self receiveMessageModel:model];
    }
    else if (msgType == ADMMessageTypeText) {
        ADMChatMessageModel *model = [ADMChatMessageManager createTextMessage:self.userModel
                                                                      message:@"[微笑]我收到了一条文本消息"
                                                                     isSender:NO];
        [self receiveMessageModel:model];
    }
    else if (msgType == ADMMessageTypeImage) {
        //收到图片
        //原图和缩略图链接
        NSString *original = @"http://www.vasueyun.cn/llgit/ADMChat/2.jpg";
        NSString *thumbnail = @"http://www.vasueyun.cn/llgit/ADMChat/2_t.jpg";
        ...
        //创建图片model
        ADMChatMessageModel *model = [ADMChatMessageManager createImageMessage:self.userModel
                                                                     thumbnail:thumbnail
                                                                      original:original
                                                                     thumImage:thumImage
                                                                      oriImage:oriImage
    ...
    else if (msgType == ADMMessageTypeVoice) {
        //接收到声音
        //声音地址
        NSString *voiceUrl = @"";
        ...
        ADMChatMessageModel *model = [ADMChatMessageManager createVoiceMessage:self.userModel
                                                                      duration:duration
                                                                      voiceUrl:voiceUrl
                                                                      isSender:NO];
        [self sendMessageModel:model];
    }
    else if (msgType == ADMMessageTypeVideo) {
        //收到视频
        NSString *videoUrl = @"";
        //封面图链接
        NSString *coverUrl = @"http://www.vasueyun.cn/llgit/ADMChat/1_t.jpg";
        //下载封面图
        UIImage *coverImage = [UIImage imageNamed:@"1_t.jpg"];
        //创建视频model
        ADMChatMessageModel *model = [ADMChatMessageManager createVideoMessage:self.userModel
                                                                      videoUrl:videoUrl
                                                                      coverUrl:coverUrl
...
- (void)inputView:(ADMInputView *)inputView sendMessage:(NSString *)message {
    //清空草稿
    [[ADMChatDBManager DBManager] removeDraftWithModel:self.userModel];
    ADMChatMessageModel *model = [ADMChatMessageManager createTextMessage:self.userModel
                                                                  message:message
                                                                 isSender:YES];
    [self sendMessageModel:model];
...
- (void)inputView:(ADMInputView *)inputView didSelectMoreType:(WZInputMoreType)type {
    if (type == WZInputMoreTypeImage) {
        ...
        //原图和缩略图链接
        NSString *original = @"http://www.vasueyun.cn/llgit/ADMChat/1.jpg";
        NSString *thumbnail = @"http://www.vasueyun.cn/llgit/ADMChat/1_t.jpg";
        ...
        //将封面图上传到服务器, 获取封面图链接
        NSString *coverUrl = @"http://www.vasueyun.cn/llgit/ADMChat/2_t.jpg";
...
- (void)inputView:(ADMInputView *)inputView didChangeText:(NSString *)text {
    //保存草稿
    [[ADMChatDBManager DBManager] setDraft:text model:self.userModel];
}

//录音状态变化
- (void)inputView:(ADMInputView *)inputView didChangeRecordType:(ADMRecordType)type {
    static NSInteger start;
    if (type == ADMRecordTypeBegin) {
        //开始录音
        start = [ADMChatHelper nowTimestamp];
    }
    else if (type == ADMRecordTypeFinish) {
        NSInteger duration = [ADMChatHelper nowTimestamp]-start;
        //结束录音
        //将录音上传到服务器, 获取录音链接
        NSString *voiceUrl = @"";
        //创建录音model
...
//发送消息
- (void)sendMessageModel:(ADMChatMessageModel *)model {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        if (i == 0) {
            model.sendType = ADMMessageSendTypeFailed;
        }
        else {
            model.sendType = ADMMessageSendTypeSuccess;
        }
        if (self.userModel) {
            [[ADMChatDBManager DBManager] updateMessageModel:model chatWithUser:self.userModel];
        }
        else {
            [[ADMChatDBManager DBManager] updateMessageModel:model chatWithGroup:self.groupModel];
        }
        [self.tableView reloadData];
    });
    
    [ADMChatNotificationManager postSessionNotification];
}

//收到消息
- (void)receiveMessageModel:(ADMChatMessageModel *)model {
    [self addMessageModel:model];
...
//消息存储
- (void)addMessageModel:(ADMChatMessageModel *)model {
    [self.messageModels addObject:model];
    [_tableView reloadData];
    [self tableViewScrollToBottom:YES duration:0.25];
    
    if (self.userModel) {
        [[ADMChatDBManager DBManager] insertMessage:model chatWithUser:self.userModel];
    }
    else {
        [[ADMChatDBManager DBManager] insertMessage:model chatWithGroup:self.groupModel];
    }
}
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.row < self.messageModels.count) {
        ADMChatMessageModel *model = [self.messageModels objectAtIndex:indexPath.row];
        [model cacheModelSize];
        if (model.msgType == ADMMessageTypeSystem) {
            return model.modelH;
        }
        if (self.isShowName) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.row < self.messageModels.count) {
        ...
        ADMChatBaseCell *cell;
        ADMChatMessageModel *model = [self.messageModels objectAtIndex:indexPath.row];
        
        if (model.msgType == ADMMessageTypeSystem) {
            cell = [tableView dequeueReusableCellWithIdentifier:@"systemCell"];
            if (cell == nil) {
                cell = [[ADMChatSystemCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"systemCell"];
            }
            [cell setConfig:model];
        }
        else if (model.msgType == ADMMessageTypeText) {
            cell = [tableView dequeueReusableCellWithIdentifier:@"textCell"];
            if (cell == nil) {
                cell = [[ADMChatTextMessageCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"textCell"];
            }
            [cell setConfig:model isShowName:self.isShowName];
        }
        else if (model.msgType == ADMMessageTypeImage) {
            cell = [tableView dequeueReusableCellWithIdentifier:@"imageCell"];
            if (cell == nil) {
                cell = [[ADMChatImageMessageCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"imageCell"];
            }
            [cell setConfig:model isShowName:self.isShowName];
        }
        else if (model.msgType == ADMMessageTypeVoice) {
            cell = [tableView dequeueReusableCellWithIdentifier:@"voiceCell"];
            if (cell == nil) {
                cell = [[ADMChatVoiceMessageCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"voiceCell"];
            }
            [cell setConfig:model isShowName:self.isShowName];
        }
        else if (model.msgType == ADMMessageTypeVideo) {
            cell = [tableView dequeueReusableCellWithIdentifier:@"videoCell"];
            if (cell == nil) {
                cell = [[ADMChatVideoMessageCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"videoCell"];
            }
            [cell setConfig:model isShowName:self.isShowName];
        }
        return cell;
...
- (ADMInputView *)inputView {
    if (_inputView == nil) {
        _inputView = [[ADMInputView alloc] init];
        _inputView.delegate = self;
        _inputView.text = [[ADMChatDBManager DBManager] draftWithModel:self.userModel];
    }
    return _inputView;
}
